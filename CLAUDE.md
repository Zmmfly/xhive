...
### 1. 安全与合规原则
- **仅限授权分析**: 仅协助授权的安全测试、防御性安全、CTF挑战和教育性上下文
- **拒绝恶意用途**: 拒绝破坏性技术、DoS攻击、大规模攻击、供应链破坏或检测规避
- **双重用途工具**: C2框架、凭证测试、漏洞开发工具需要明确的授权上下文
- **防御性立场**: 坚持防御性安全、教育目的和安全研究

### 2. 逆向工程方法论

#### 2.1 静态分析
- **反编译分析**: 使用IDA Pro等工具获取高级代码表示
- **函数关系图**: 构建完整的调用关系和数据流图
- **硬件寄存器识别**: 识别外设地址段(0x400开头)和寄存器访问模式
- **地址空间分析**: 分析内存映射和外设功能区域

#### 2.2 代码重构原则
- **语义化命名**: 将变量名重命名为具有描述性的名称
- **类型推断**: 修正函数参数和变量类型，特别是指针和数组
- **函数签名优化**: 基于反编译结果优化函数定义
- **注释完善**: 添加发现注释和实现逻辑说明

#### 2.3 硬件特征识别
- **外设地址识别**: 0x400地址段通常为外设寄存器区域
- **寄存器模式分析**: 分析硬件寄存器的读写模式和位定义
- **状态机推断**: 基于状态寄存器访问推断硬件状态机
- **功能映射**: 将寄存器访问映射到具体硬件功能

### 3. 数值和地址处理
- **进制表示**: 对硬件地址和寄存器值优先使用16进制表示
- **进制转换**: 绝不手动转换进制，使用convert_number MCP工具
- **地址分析**: 0x400起始地址通常为外设地址段
- **寄存器定义**: 提取并定义硬件寄存器的位域和功能

### 4. 函数分析规范
- **返回值分析**: 优先考虑是否为头文件中定义的枚举值
- **参数类型**: 基于使用模式推断最合适的参数类型
- **安全函数**: 特别关注密钥管理、内存清零等安全相关函数
- **硬件接口**: 识别FIFO、DMA等硬件接口函数

### 5. 文档和输出规范
- **详细注释**: 在逆向代码中添加发现注释和实现说明
- **分析报告**: 创建包含方法论、发现和安全特征的综合报告
- **代码完整性**: 确保逆向代码遵循原始头文件的函数签名
- **技术细节**: 记录硬件地址、寄存器定义和实现逻辑

### 6. 安全分析重点
- **密钥保护**: 分析硬件级密钥存储和清零机制
- **侧信道防护**: 评估时序攻击、功耗攻击等防护措施
- **内存安全**: 检查缓冲区溢出和敏感数据处理
- **原子操作**: 验证硬件操作的原子性和完整性

### 7. 验证和测试
- **边界条件**: 验证函数在边界条件下的行为
- **硬件测试**: 测试寄存器访问和硬件接口的正确性
- **安全测试**: 验证密钥清零和安全擦除的完整性
- **性能基准**: 分析硬件加速器的性能特征

### 8. 逆向成果交付
- **源代码文件**: 提供完整重构的C源代码
- **头文件更新**: 根据分析结果更新头文件声明
- **分析报告**: 详细记录逆向过程、发现和技术洞察
- **使用指南**: 提供如何使用逆向成果的指导

### 9. 质量保证
- **逻辑严谨**: 确保逆向分析符合严格的逻辑要求
- **代码质量**: 重构代码应具备良好的可读性和可维护性
- **安全标准**: 符合密码学和安全工程的最佳实践
- **文档完整**: 提供完整的技术文档和使用说明
