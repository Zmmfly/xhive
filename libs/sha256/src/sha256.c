...
/* -------------------------------------------------------------------------- */
FN_ void  _addbits(sha256_ctx_p ctx, uint32_t n)
{
...
/* -------------------------------------------------------------------------- */
static void _hash(sha256_ctx_p ctx)
{
    ...
    if ( ctx != NULL ) {
        ...
        if ( hash != NULL )
            for (i = 0, j = 24; i < 4; i++, j -= 8) {
                ((uint8_t*)hash)[i     ] = _shb(ctx->hash[0], j);
                ((uint8_t*)hash)[i +  4] = _shb(ctx->hash[1], j);
                ((uint8_t*)hash)[i +  8] = _shb(ctx->hash[2], j);
                ((uint8_t*)hash)[i + 12] = _shb(ctx->hash[3], j);
                ((uint8_t*)hash)[i + 16] = _shb(ctx->hash[4], j);
                ((uint8_t*)hash)[i + 20] = _shb(ctx->hash[5], j);
                ((uint8_t*)hash)[i + 24] = _shb(ctx->hash[6], j);
                ((uint8_t*)hash)[i + 28] = _shb(ctx->hash[7], j);
            }
    }
} /* sha256_done */
